...
function resetForm() {
  ...
  document
    .querySelectorAll(
      "#agreedSalary, #insuranceSalary, #otherAllowances, #otherDeductions, #currentCash"
    )
    .forEach((el) => formatNumberInput(el));
...
function calculateExpenses() {
  ...
  expenseRows.forEach((row) => {
    ...
    const amount = parseFloat(
      (amountInput && amountInput.value.replace(/\./g, "")) || "0"
    );
  ...
  if (totalExpensesEl)
    totalExpensesEl.innerText = `- ${formatCurrency(totalExpenses)}`;
...
function addExpenseRow() {
  ...
  newRow.className =
    "expense-row sm:flex sm:items-center sm:gap-x-3 space-y-2 sm:space-y-0 animate-fade-in";
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:gap-x-3 flex-1">
    <input type="text" placeholder="Nội dung chi (ví dụ: Tiền nhà)" 
      class="expense-description flex-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm mb-2 sm:mb-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600">
    <input type="text" inputmode="numeric" placeholder="Số tiền" 
      class="expense-amount flex-1 h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600">
  </div>
  <div class="flex-shrink-0">
    <button 
  type="button" 
  onclick="this.closest('.expense-row').remove(); updateAllCalculations();" 
  class="remove-expense-btn inline-flex items-center gap-2 rounded-md bg-red-600 px-3 py-2 text-sm font-medium text-white hover:bg-red-700 transition-colors"
>
...
`;
  ...
  newRow
    .querySelector("input.expense-description")
...
function calculateAll() {
  ...
  const actualWorkdays =
    parseFloat(document.getElementById("actualWorkdays").value) || 0;
  const paidLeaveDays =
    parseFloat(document.getElementById("paidLeaveDays").value) || 0;
  ...
  if (otherUnitEl && otherUnitEl.value === "percent") {
    ...
    const pctRaw =
      rawOtherInput && rawOtherInput.value
        ? rawOtherInput.value.replace(/,/g, ".")
        : "0";
  ...
  const paidLeaveSalary =
    paidLeaveDays > 0 ? (insuranceSalary / 26) * paidLeaveDays : 0;
  ...
  let taxableIncome = Math.max(
    0,
    totalIncome - totalInsurance - personalDeduction - dependentDeduction
  );
  ...
  document.getElementById("res-workday-salary").innerText =
    formatCurrency(workdaySalary);
  document.getElementById(
    "res-actual-workdays-detail"
  ).innerText = `(${actualWorkdays} ngày)`;
  document.getElementById("res-paid-leave-salary").innerText =
    formatCurrency(paidLeaveSalary);
  document.getElementById(
    "res-paid-leave-detail"
  ).innerText = `(${paidLeaveDays} ngày)`;
  document.getElementById("res-allowances").innerText = `+ ${formatCurrency(
    otherAllowances
  )}`;
  document.getElementById("res-gross-salary").innerText =
  ...
  document.getElementById(
    "res-other-deductions"
  ).innerText = `- ${formatCurrency(otherDeductions)}`;
  document.getElementById("res-total-deductions").innerText = `${formatCurrency(
    totalDeductions
  )}`;
  document.getElementById("res-net-salary").innerText =
...
window.addEventListener("load", function () {
  ...
  (function setupOtherDeductionsDropdown() {
    ...
    function openList() {
      ...
      document
        .getElementById("otherDeductionsDropdown")
    ...
    function setOtherDeductionsUnit(val, itemEl) {
      ...
      if (typeof validateOtherDeductions === "function")
        validateOtherDeductions();
    }
    ...
    trigger.addEventListener("keydown", function (e) {
      if (e.key === "ArrowDown" || e.key === "Down") {
        e.preventDefault();
        if (!open) openList();
        else focusItem((focusedIndex + 1) % items.length);
      } else if (e.key === "ArrowUp" || e.key === "Up") {
        e.preventDefault();
        if (!open) openList();
        else focusItem((focusedIndex - 1 + items.length) % items.length);
      } else if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        if (!open) openList();
        else selectItem(focusedIndex >= 0 ? focusedIndex : 0);
    ...
    document.addEventListener("click", function (ev) {
      if (
        !document.getElementById("otherDeductionsDropdown").contains(ev.target)
      ) {
        if (open) closeList();
      }
    });
...
function openExportModal() {
  ...
  if (!modal._cleanupFocusTrap)
    modal._cleanupFocusTrap = trapFocusInModal(modal);
}
...
function trapFocusInModal(modalEl) {
  ...
  const nodes = Array.from(modalEl.querySelectorAll(focusableSelector)).filter(
...
function buildSalarySheet() {
  ...
  const otherUnitEl = document.getElementById("otherDeductionsUnit");
  const otherRaw = document.getElementById("otherDeductions")
  ...
  if (otherUnitEl && otherUnitEl.value === "percent") {
    ...
    const insuranceSalary =
      getNumericValue("insuranceSalary") || getNumericValue("agreedSalary");
  ...
  if (!document.getElementById("results").classList.contains("hidden")) {
    ...
    resultIds.forEach((r) =>
      rows.push([r[0], document.getElementById(r[1]).innerText || ""])
    );
  }
...
function buildExpenseSheet() {
  ...
  expenseRows.forEach((row, i) => {
    const desc = row.querySelector(".expense-description")
      ? row.querySelector(".expense-description").value
      : "";
    const amt = row.querySelector(".expense-amount")
  ...
  rows.push([
    "Tổng quỹ",
    document.getElementById("summary-total-funds")
      ? document.getElementById("summary-total-funds").innerText
      : "",
  ]);
  rows.push([
    "Tổng chi tiêu",
...
document.addEventListener("click", function (e) {
  const btn =
    e.target && e.target.closest
      ? e.target.closest('button, [role="button"], [data-action]')
      : null;
  if (btn) {
    ...
    if (id === "export-cancel" || id === "export-cancel-2") {
      closeExportModal();
      return;
    }
    if (id === "export-confirm") {
  ...
  if (
    e.target &&
    e.target.classList &&
    e.target.classList.contains("export-backdrop")
  ) {
    const modal = document.getElementById("export-modal");
